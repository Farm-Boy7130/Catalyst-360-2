...
void ID22()
{
    ...
    SetConsoleTitle("Catalyst 360 | Displacement Calculator 2022 (Release)");
    ...
    std::string InvalidResponse = " is an invalid response, Please try again. Please make sure to use either a 1 or a 2. Please try again.";
    ...
    std::cout << "Please input your Bore in " << Measurement_Type << "." << std::endl;
    std::cout << "User Answer: ";
    std::cin >> Bore;
    ...
    std::string path;
    if (userpath != nullptr) {
        path = std::string(userpath) + "\\Desktop";
    }
    ...
    if ((Measurement_System == "1") && (ResultFilePrompt == "y")) // Printing Imperial System File results.
    {
        ...
        std::string CIs = std::to_string(CI_Displacement);
        std::string LD = std::to_string(Liters_Displacement);
        time_t rawtime;
        struct tm* timeinfo;
        char buffer[100];
        time(&rawtime);
        timeinfo = localtime(&rawtime);
        strftime(buffer, 100, "(Imperial System) Catalyst 360 Displacement Calculator results on: %c", timeinfo);
        std::cout << buffer;
        std::fstream resultfile;
        resultfile.open(path + "\\Results.txt", std::fstream::in | std::fstream::out | std::fstream::app);
        if (resultfile.is_open())
        {
            ...
            resultfile << LD;
            resultfile << "\n\nCopyright (c) 2023 Farm-Boy7130\n\n"; // Extra line break to leave gap for Future Results.
            resultfile.close();
            std::cout << "Created/ or Edited a text file on the desktop with your results.";
        }
        else
        {
            std::cout << "\nError Opening/Closing and or printing to file. Please try again later.";
        }
    }
    ...
    {
        double CC_Displacement = ((0.7854 * Bore * Bore) * Stroke * Cylinder_Count) / 1000; // Maths
        std::string CCs = std::to_string(CC_Displacement);
        time_t rawtime;
        struct tm* timeinfo;
        char buffer[100];
        time(&rawtime);
        timeinfo = localtime(&rawtime);
        strftime(buffer, 100, "(Metric System) Catalyst 360 Displacement Calculator results on: %c", timeinfo);
        std::cout << buffer;
        std::fstream resultfile;
        resultfile.open(path + "\\Results.txt", std::fstream::in | std::fstream::out | std::fstream::app);
        if (resultfile.is_open())
    ...
    {
        std::cout << "No result file created. Press any key to continue... \n\nCopyright (c) 2023 Farm-Boy7130" << std::endl;
        system("pause");
    }
    ...
    std::cout << "\n\nCopyright (c) 2023 Farm-Boy7130\n";
    std::cout << "Executed all code and exited with code 0\n";
    system("pause");
